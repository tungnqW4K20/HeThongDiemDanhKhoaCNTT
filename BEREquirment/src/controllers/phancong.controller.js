...
const getLichGiangDay = async (req, res) => {
    ...
    const hocKy = await db.HocKy.findByPk(hocky_id, {
      attributes: ['hocky_id', 'ten_hocky', 'ngay_batdau', 'ngay_ketthuc', 'ngay_monday_tuan_1']
    });
...
const importSchedule = async (req, res) => {
    try {
        ...
        const { ten_hocky, ngay_batdau, ngay_ketthuc, ngay_monday_tuan_1 } = req.body;
        ...
        const result = await phanCongService.importScheduleExcel(req.file.buffer, {
            ten_hocky, ngay_batdau, ngay_ketthuc, ngay_monday_tuan_1
        });
