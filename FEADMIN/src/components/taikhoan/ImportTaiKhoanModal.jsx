...
const ImportTaiKhoanModal = ({ isOpen, onClose, onImport, isLoading }) => {
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      { 
        "Tên đăng nhập": "gv_an_nguyen", 
        "Mật khẩu": "MatKhau123", 
        "Vai trò": "giangvien", 
        "Mã giảng viên (nếu có)": "GV001",
        "Mã Bộ môn quản lý (nếu có)": "",
        "Mã Khoa quản lý (nếu có)": ""
      },
      { 
        "Tên đăng nhập": "tbm_cntt", 
        "Mật khẩu": "MatKhau123", 
        "Vai trò": "truongbomon", 
        "Mã giảng viên (nếu có)": "GV002",
        "Mã Bộ môn quản lý (nếu có)": "CNTT",
        "Mã Khoa quản lý (nếu có)": ""
      },
      { 
        "Tên đăng nhập": "ld_cntt", 
        "Mật khẩu": "MatKhau123", 
        "Vai trò": "lanhdao", 
        "Mã giảng viên (nếu có)": "",
        "Mã Bộ môn quản lý (nếu có)": "",
        "Mã Khoa quản lý (nếu có)": "CNTT"
      },
    ];
    ...
    // Set độ rộng cột
    ws['!cols'] = [
        { wch: 18 }, { wch: 15 }, { wch: 15 }, { wch: 22 }, { wch: 26 }, { wch: 24 }
    ];
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex items-start gap-3">
             ...
             <div className="flex-1">
                ...
                <p className="text-xs text-gray-600 mt-1 mb-3">
                   Tải file mẫu chuẩn để nhập liệu chính xác. Vai trò hợp lệ: <code className="bg-blue-100/50 px-1 rounded text-red-600 font-mono text-[10px]">admin</code>, <code className="bg-blue-100/50 px-1 rounded text-red-600 font-mono text-[10px]">giangvien</code>, <code className="bg-blue-100/50 px-1 rounded text-red-600 font-mono text-[10px]">truongbomon</code>, <code className="bg-blue-100/50 px-1 rounded text-red-600 font-mono text-[10px]">lanhdao</code>.
                </p>
                <button 
                  onClick={handleDownloadTemplate}
                  className="px-3 py-1.5 bg-white border border-blue-200 text-[#3B5998] text-xs font-bold rounded shadow-sm hover:bg-blue-50 flex items-center gap-2 transition-all cursor-pointer"
                >
...
export default ImportTaiKhoanModal;
