    ...
    <>
      {[...Array(5)].map((_, index) => (
        <tr key={index} className="animate-pulse border-b border-gray-100 last:border-0">
          <td className="p-4"><div className="h-4 w-4 bg-gray-200 rounded mx-auto"></div></td>
          ...
          <td className="px-4 py-3"><div className="h-4 bg-gray-200 rounded w-full mb-1"></div><div className="h-3 bg-gray-200 rounded w-1/2"></div></td>
          <td className="px-4 py-3"><div className="flex justify-center gap-2"><div className="h-8 w-8 bg-gray-200 rounded"></div><div className="h-8 w-8 bg-gray-200 rounded"></div></div></td>
        </tr>
      ))}
    </>
);
...
const LecturerTable = ({ 
    ...
    // Khi danh sách gốc thay đổi (do search/filter ở cha) -> Reset về trang 1
    useEffect(() => {
        resetPagination();
    }, [lecturers]);
    ...
    const handleSelectAll = (e) => {
        if (onSelectionChange) {
            // Select tất cả trong danh sách đã lọc (sortedLecturers) chứ không chỉ trang hiện tại
    ...
    // Kiểm tra select all dựa trên sortedLecturers
    const isAllSelected = sortedLecturers.length > 0 && 
    ...
    return (
        <div className="w-full flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
             {/* Container cho table có scroll ngang nếu màn hình nhỏ */}
             <div className="overflow-x-auto custom-scrollbar">
                <table className="min-w-full table-fixed divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {isLoading ? <TableSkeleton /> : currentData.length > 0 ? (
                            currentData.map((gv) => {
                                const isSelected = selectedIds.includes(gv.giangvien_id);
                                return (
                                    <tr 
                                        key={gv.giangvien_id} 
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 align-middle">
                                            <div className="flex items-center gap-3">
                                                <Avatar firstName={gv.ten} lastName={gv.ho} />
