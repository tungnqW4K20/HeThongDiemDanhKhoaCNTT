...
const LecturerModal = ({ isOpen, onClose, onSave, initialData, faculties = [], boMonOptions = [] }) => {
    ...
    if (isOpen) {
      if (initialData) {
        ...
        setFormData({
          ...
          bomon_ids: initialData.DanhSachBoMon ? initialData.DanhSachBoMon.map(bm => bm.bomon_id) : [],
          ho: initialData.ho || '',
          ten: initialData.ten || '',
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              {/* Danh sách Bộ môn */}
              <div className="md:col-span-3 mt-2 p-4 bg-gray-50 border border-gray-100 rounded-lg">
                <label className="block text-xs font-bold text-gray-600 mb-3">Phân công Bộ môn</label>
                {!formData.khoa_id ? (
                  <span className="text-sm text-gray-400 italic">Vui lòng chọn Khoa/Viện trước để hiển thị danh sách bộ môn.</span>
                ) : (
                  <div className="flex flex-wrap gap-3">
                    {boMonOptions?.filter(bm => bm.khoa_id === formData.khoa_id).length === 0 && (
                      <span className="text-sm text-gray-400 italic">Khoa này chưa có bộ môn nào.</span>
                    )}
                    {boMonOptions?.filter(bm => bm.khoa_id === formData.khoa_id).map(bm => (
                      <label key={bm.id} className="flex items-center gap-2 cursor-pointer bg-white px-3 py-2 border border-gray-200 rounded hover:border-[#3B5998]/50 transition-colors">
                        <input
                          type="checkbox"
                          className="w-4 h-4 text-[#3B5998] rounded border-gray-300 focus:ring-[#3B5998]"
                          checked={formData.bomon_ids.includes(bm.id)}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setFormData({ ...formData, bomon_ids: [...formData.bomon_ids, bm.id] });
                            } else {
                              setFormData({ ...formData, bomon_ids: formData.bomon_ids.filter(id => id !== bm.id) });
                            }
                          }}
                        />
                        <span className="text-sm text-gray-700 font-medium">{bm.name}</span>
                      </label>
                    ))}
                  </div>
                )}
