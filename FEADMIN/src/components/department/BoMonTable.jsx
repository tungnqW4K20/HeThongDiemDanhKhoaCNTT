...
import { Edit2, Trash2, Loader2, Search, Library } from 'lucide-react';

const BoMonTable = ({ data, loading, onEdit, onDelete, canEdit = true, searchTerm, setSearchTerm }) => {
  if (loading) {
    return (
      <div className="py-24 flex flex-col items-center justify-center text-gray-400">
        <Loader2 className="animate-spin text-[#3B5998] mb-3" size={40} />
        <p className="animate-pulse">Đang tải danh sách bộ môn...</p>
      </div>
    );
  }
      ...
      <div className="bg-white border-b border-gray-100 p-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-4 text-sm font-medium text-gray-500">
          <div className="flex items-center gap-2">
            ...
            <span>
              Tổng số: <b className="text-gray-800">{data.length}</b>
            </span>
          </div>
        </div>
      </div>
      ...
      <table className="w-full text-left">
        <thead className="bg-gray-50/50 border-b border-gray-100">
          <tr>
            ...
            <th className="px-6 py-4 font-bold text-[#3B5998] text-xs uppercase tracking-wider w-36">Mã Bộ môn</th>
            <th className="px-6 py-4 font-bold text-[#3B5998] text-xs uppercase tracking-wider">Tên Bộ môn</th>
              ...
              <tr key={item.chuyennganh_id} className="hover:bg-gray-50/50 transition-colors group">
                ...
                <td className="px-6 py-4">
                  <div className="text-sm font-semibold text-gray-700">
                    {item.TruongBoMon?.GiangVien ? `${item.TruongBoMon.GiangVien.ho || ''} ${item.TruongBoMon.GiangVien.ten || ''}`.trim() : 'Chưa phân công'}
                  </div>
