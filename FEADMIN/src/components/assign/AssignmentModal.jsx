    ...
    const fetchLecturersByFaculty = async () => {
        ...
        if (selectedSubject && selectedSubject.Khoa) {
            try {
                ...
                if (response && response.data) setLecturers(response.data);
                else setLecturers([]);
            } catch (error) {
                console.error(error);
                setLecturers([]);
    ...
    if (isOpen) {
      if (initialData) {
        setFormData({
            ...
            so_tuan: initialData.so_tuan || initialData.so_tiet || 15
        });
        ...
        setFormData({
          ...
          giangvien_id: '',
          lop_hanhchinh_ids: [],
          thu: currentDay, // Set thứ theo ngày mặc định
          gio_batdau: '07:00',
          gio_ketthuc: '09:30',
          phong: '',
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             ...
             <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <InputGroup label="Giờ bắt đầu" icon={Clock}>
                        <input 
                            type="time" 
                            value={formData.gio_batdau}
                            onChange={(e) => setFormData({...formData, gio_batdau: e.target.value})}
                            className="w-full pl-10 pr-2 py-2.5 border border-gray-200 rounded-lg focus:border-[#3B5998] text-sm"
                        />
                    </InputGroup>
                    
                    <InputGroup label="Giờ kết thúc" icon={Clock} error={errors.gio}>
                        <input 
                            type="time" 
                            value={formData.gio_ketthuc}
                            onChange={(e) => setFormData({...formData, gio_ketthuc: e.target.value})}
                            className="w-full pl-10 pr-2 py-2.5 border border-gray-200 rounded-lg focus:border-[#3B5998] text-sm"
                        />
                    </InputGroup>
                </div>
