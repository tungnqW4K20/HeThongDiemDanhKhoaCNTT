...
const generateGlobalWeeks = (allSemestersInYear) => {
    if (!allSemestersInYear || allSemestersInYear.length === 0) return [];

    // 1. Reset: Sắp xếp các học kỳ trong năm đó để tìm ngày bắt đầu sớm nhất
    const sortedSemesters = [...allSemestersInYear].sort((a, b) => 
        new Date(a.ngay_batdau) - new Date(b.ngay_batdau)
    );

    // 2. Tìm Thứ 2 mốc sớm nhất của NĂM ĐÓ (Đây chính là Tuần 1 của năm mới)
    const allMondays = allSemestersInYear.map(s => new Date(s.ngay_monday_tuan_1).getTime());
    const minMondayDate = new Date(Math.min(...allMondays));
    
    let currentMonday = new Date(minMondayDate);

    // 3. Tìm ngày kết thúc muộn nhất của NĂM ĐÓ
    const allEndDates = allSemestersInYear.map(s => new Date(s.ngay_ketthuc).getTime());
    const yearEndDate = new Date(Math.max(...allEndDates));

    const globalWeeks = [];
    let weekIndex = 1; // LUÔN KHỞI TẠO LÀ 1 CHO MỖI NĂM HỌC

    while (currentMonday <= yearEndDate) {
        const nextSunday = new Date(currentMonday);
        nextSunday.setDate(currentMonday.getDate() + 6);
        nextSunday.setHours(23, 59, 59, 999);

        globalWeeks.push({
            id: weekIndex, // Số thứ tự tuần (1, 2, 3...)
            label: `Tuần ${weekIndex}`,
            rangeText: `(${formatDateVN(currentMonday)} - ${formatDateVN(nextSunday)})`,
            startDate: new Date(currentMonday),
            endDate: nextSunday,
...
export default function AssignmentPage() {
    ...
    useEffect(() => {
        ...
        // CHỐT CHẶN: Chỉ lấy các học kỳ thuộc cùng Năm học (namhoc_id)
        // Ví dụ: Đang ở năm 2026-2027 thì chỉ lấy các học kỳ của 2026-2027
        const semestersInSameYear = semesters.filter(s => s.namhoc_id === currentSem.namhoc_id);

        // Tính toán lại toàn bộ danh sách tuần cho năm này (Bắt đầu từ 1)
        const weeksOfThisYear = generateGlobalWeeks(semestersInSameYear);
        setWeeks(weeksOfThisYear);

        // Nhảy đến tuần tương ứng trong năm đó
        const targetWeek = weeksOfThisYear.find(w => w.mondayStr === currentSem.ngay_monday_tuan_1);

        if (targetWeek) {
            setSelectedWeek(targetWeek.id);
        } else {
            // Nếu không khớp ngày Thứ 2, tìm tuần chứa ngày bắt đầu học kỳ
            const startDate = new Date(currentSem.ngay_batdau);
            const fallbackWeek = weeksOfThisYear.find(w => startDate >= w.startDate && startDate <= w.endDate);
            setSelectedWeek(fallbackWeek ? fallbackWeek.id : 1);
        }
    }, [currentSemesterId, semesters]);
