...
const Overview = () => {
  ...
  const [chartFilter, setChartFilter] = useState('week'); // 'day' | 'week' | 'month' | 'year'

  const currentYear = new Date().getFullYear();
  const currentMonth = new Date().getMonth() + 1;
  const [selectedYear, setSelectedYear] = useState(currentYear);
  const [selectedMonth, setSelectedMonth] = useState(currentMonth);
  const yearOptions = Array.from({ length: currentYear - 2022 }, (_, i) => currentYear - i); // 2023 → now
  ...
  const fetchChartData = async (filter, year, month) => {
    setChartLoading(true);
    try {
      const res = await dashboardService.getChartData(filter, year, month);
  ...
  const handleChartFilterChange = (filterType) => {
    if (filterType === chartFilter) return;
    setChartFilter(filterType);
    fetchChartData(filterType, selectedYear, selectedMonth);
  };

  const handleYearChange = (y) => {
    const yr = parseInt(y);
    setSelectedYear(yr);
    // Năm hiện tại → reset về tháng hiện tại; năm cũ → về tháng 1
    const mo = yr === currentYear ? currentMonth : 1;
    setSelectedMonth(mo);
    fetchChartData(chartFilter, yr, mo);
  };

  const handleMonthChange = (m) => {
    const mo = parseInt(m);
    setSelectedMonth(mo);
    fetchChartData(chartFilter, selectedYear, mo);
  };
  ...
  const getFilterLabel = () => {
    switch(chartFilter) {
      ...
      case 'month': return `${MONTH_NAMES[selectedMonth-1]} ${selectedYear}`;
  ...
  const getFilterDesc = () => {
    switch(chartFilter) {
      case 'day':   return 'Tỷ lệ điểm danh theo từng lớp trong ';
      case 'week':  return 'Tỷ lệ điểm danh theo từng ngày trong ';
      case 'month': return 'Tỷ lệ điểm danh theo từng tuần trong ';
      case 'year':  return 'Tỷ lệ điểm danh theo từng tháng trong ';
      default: return '';
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                      ...
                      { id: 'year',  label: 'Năm' }
                        ...
                        <button 
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all ${
                              chartFilter === item.id ? 'bg-white text-[#3B5998] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                            }`}
                        >
                            {item.label}
                        </button>
                ...
                {/* Chọn tháng (chỉ khi filter = month) */}
                {chartFilter === 'month' && (
                    <select
                        value={selectedMonth}
                        onChange={(e) => handleMonthChange(e.target.value)}
                        className="text-sm border border-slate-200 rounded-lg px-2 py-1.5 bg-white text-slate-700 focus:outline-none focus:ring-1 focus:ring-[#3B5998] cursor-pointer"
                    >
                        {MONTH_NAMES.map((name, i) => {
                            const m = i + 1;
                            const isDisabled = selectedYear === currentYear && m > currentMonth;
                            return <option key={m} value={m} disabled={isDisabled}>{name}</option>;
                        })}
                    </select>
                )}

                {/* Chọn năm (khi filter = month hoặc year) */}
        ...
        <div className="w-full h-[400px]">
            {chartData.length > 0 ? (
                chartFilter === 'day' ? (
                    /* Hôm nay: horizontal bars — mỗi lớp 1 thanh */
                    <div style={{ height: `${Math.max(400, chartData.length * 60)}px`, width: '100%' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart layout="vertical" data={chartData} margin={{ top: 5, right: 30, left: 40, bottom: 5 }} barSize={24}>
                                ...
                                <Legend verticalAlign="top" height={36} iconType="circle" formatter={(v) => <span className="text-slate-600 font-medium ml-1">{v}</span>} />
                                <Bar name="Có mặt" dataKey="present" stackId="a" fill="#10b981"><LabelList dataKey="present" content={renderCustomLabel} /></Bar>
                                <Bar name="Đi muộn / Phép" dataKey="late" stackId="a" fill="#f59e0b"><LabelList dataKey="late" content={renderCustomLabel} /></Bar>
                                <Bar name="Vắng không phép" dataKey="absent" stackId="a" fill="#ef4444" radius={[0, 4, 4, 0]}><LabelList dataKey="absent" content={renderCustomLabel} /></Bar>
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                ) : (
                    /* Tuần / Tháng: vertical column chart — mỗi ngày/tuần 1 cột */
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }} barSize={48}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                            <XAxis dataKey="name" tick={{ fill: '#475569', fontSize: 13, fontWeight: 500 }} axisLine={false} tickLine={false} />
                            <YAxis domain={[0, 100]} unit="%" tick={{ fill: '#94a3b8', fontSize: 12 }} axisLine={false} tickLine={false} width={40} />
                            <Tooltip content={<CustomTooltip />} cursor={{ fill: '#f1f5f9' }} />
                            <Legend verticalAlign="top" height={36} iconType="circle" formatter={(v) => <span className="text-slate-600 font-medium ml-1">{v}</span>} />
                            <Bar name="Có mặt" dataKey="present" stackId="a" fill="#10b981"><LabelList dataKey="present" content={renderCustomLabel} /></Bar>
                            <Bar name="Đi muộn / Phép" dataKey="late" stackId="a" fill="#f59e0b"><LabelList dataKey="late" content={renderCustomLabel} /></Bar>
                            <Bar name="Vắng không phép" dataKey="absent" stackId="a" fill="#ef4444" radius={[4, 4, 0, 0]}><LabelList dataKey="absent" content={renderCustomLabel} /></Bar>
                        </BarChart>
                    </ResponsiveContainer>
                )
            ) : (
